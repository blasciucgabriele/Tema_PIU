...
namespace Farmacie
{
	public class Nomenclator:Medicament
	{
	
		public int nr_medicamente = 0;
		...
		public void adauga_produs_cmd(string[] c)
		{
			if (!isFull())
			{
				med[nr_medicamente] = new Medicament(c[0], c[1], c[2], int.Parse(c[3]), float.Parse(c[4]), int.Parse(c[5]), c[6]);
				nr_medicamente++;
				Console.WriteLine("Produs adaugat cu succes!");
			}
			else Console.WriteLine("Baza de date este plina!");
		...
		public bool validare_nume(string _nume)
		{
			for (int i = 0; i < nr_medicamente; i++)
			{
				if (String.Compare(_nume, med[i].denumire) == 0)
		...
		public int get_index(string _nume)
		{
			for (int i = 0; i < nr_medicamente; i++)
			{
				if (String.Compare(_nume, med[i].denumire) == 0)
		...
		public string afisare_medicamente()
		{
			string buf="";
			if (isEmpty()) return "Nu exista produse in baza de date!";
			for (int i = 0; i < nr_medicamente; i++)
			{
				buf=buf+Convert.ToString(i+1)+".   "+med[i].afisare()+"\n";
			}
			return buf;
		}
		...
		{  
			using (StreamReader sr = new StreamReader("C:\\Users\\gabri\\Desktop\\Tema_PIU-master\\Tema_PIU-master\\Farmacie\\medicamente.txt"))
			{
				string linie;
		...
		{
			using (StreamWriter sr = new StreamWriter("C:\\Users\\gabri\\Desktop\\Tema_PIU-master\\Tema_PIU-master\\Farmacie\\medicamente_out.txt", true))
			{
				for (int i = 0; i < nr_medicamente; i++)
				{
